...
Page({
	...
	add: function (obj) {
		...
		/**
		 * 因为更改一个列表的菜品可能会影响其他列表
		 * 即一个菜品会存在于多个列表，因此必须整体查询
	...
	fixDishLikeNum() {
		app.getDishLikeNumList((res)=> {
			...
			this.addDishLineNum(hotDishList, l);
			this.addDishLineNum(recommendDishList, l);
			this.addDishLineNum(newDishList, l);
			this.addDishLineNum(combos, l);
		}, (err) => {
			app.dialog(this, "获取信息出错", "请检查您的网络是否正确连接");
			wx.hideLoading();
		})
	...
	/**
	 * 获取用户对于该菜品的喜欢，收藏和待选状态
	 * 将菜品数据与用户数据分离，菜品数据可以利用缓存直接返回 
	 */
